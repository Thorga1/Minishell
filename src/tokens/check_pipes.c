...
/*   Updated: 2025/03/06 17:14:50 by tordner          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	validate_pipes(char **tokens)
{
	...
	while(tokens[i + 1] != NULL)
		i++;
	if (classify_token(tokens[0]) == TOKEN_PIPE || classify_token(tokens[i]) == TOKEN_PIPE)
		return (1);
	i = 0;
	while (tokens[i])
	{
		if (classify_token(tokens[i]) == TOKEN_PIPE && tokens[i + 1] && classify_token(tokens[i + 1]) == TOKEN_PIPE)
			return (1);
			i++;
	}
	i = 0;
