#include "../includes/minishell.h"

static char *create_git_prompt(char *dir_name, int exit_status)
{
    char *git_branch;
    char *prompt;
    size_t prompt_size;

    git_branch = get_git_branch();
    if (!git_branch)
        return (NULL);
    prompt_size = strlen(dir_name) + strlen(git_branch) + 100;
    prompt = malloc(prompt_size);
    if (prompt)
        snprintf(prompt, prompt_size, (exit_status == 0) ? 
            GREEN PROMPT_GIT : RED PROMPT_GIT, dir_name, git_branch);
    free(git_branch);
    return (prompt);
}

static char *create_standard_prompt(char *dir_name, int exit_status)
{
    char *prompt;
    size_t prompt_size;

    prompt_size = strlen(dir_name) + 100;
    prompt = malloc(prompt_size);
    if (prompt)
        snprintf(prompt, prompt_size, (exit_status == 0) ? 
            GREEN PROMPT_STD : RED PROMPT_STD, dir_name);
    return (prompt);
}

static char *generate_prompt(t_shell *shell)
{
    char *dir_name;
    char *prompt;

    dir_name = get_current_dir_name();
    if (!dir_name)
        return (strdup("➜  ❯ "));

    if (is_git_repository())
        prompt = create_git_prompt(dir_name, shell->exit_status);
    else
        prompt = create_standard_prompt(dir_name, shell->exit_status);

    free(dir_name);
    return (prompt ? prompt : strdup("➜  ❯ "));
}

static void handle_input(char *input, t_shell *shell)
{
    int i;

    i = 0;
    while (input[i] && (input[i] == ' ' || input[i] == '\t'))
        i++;
    
    if (input[i] != '\0')
    {
        add_history(input);
        process_input(input, shell);
    }
}

void minishell_loop(t_shell *shell)
{
    char *input;
    char *prompt;

    while (shell->running)
    {
        prompt = generate_prompt(shell);
        input = readline(prompt);
        free(prompt);

        if (!input)
        {
            printf("exit\n");
            break;
        }
        handle_input(input, shell);
        free(input);
    }
}
